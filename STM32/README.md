...
及H1～H3；随后采集5 MSPS波形帧并向`s0`发送时域波形。波形下限
在压缩为原高度1/3后整体上移20个纵坐标单位。发送前将256点线性插值为
`s0.w`返回的实际控件宽度，使一个完整周期正好铺满控件；宽度读取失败时
才回退为512点。不再进行水平循环移位，当前仍不向`s1`发送频谱图。

周期按钮在1周期和3周期之间切换。测量进行中切换只更新待应用值，不会
重启或打断FPGA采集；波形帧到达后使用最后一次选择生成波形。结果保持
期间切换则直接使用缓存的5 MSPS帧重画，不重新采集。开始和周期按钮各自
具有120ms防抖，测量中重复按开始会被忽略。
